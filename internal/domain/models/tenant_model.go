...
import (
	"hyberica.io/go/go-api/internal/domain/enums"
)

type Tenant struct {
	BaseModel
	Type         enums.PersonType `gorm:"type:person_type;not null" validate:"required,oneof=FISICA JURIDICA" json:"type"`
	Name         string           `gorm:"type:varchar(100);not null" validate:"required" json:"name"`
	CpfCnpj      *string          `gorm:"type:varchar(18)" validate:"omitempty,cpfcnpj" json:"cpf_cnpj"`
	...
	Subdomain    string           `gorm:"type:varchar(20);not null;unique" validate:"required" json:"subdomain"`
	Domain       *string          `gorm:"type:varchar(100);unique" validate:"omitempty,url" json:"domain"`
	Status       enums.StatusType `gorm:"type:status_type;not null;default:'ATIVO'" validate:"required,oneof=ATIVO INATIVO" json:"status"`
}
